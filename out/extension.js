...
function activate(context) {
    ...
    const provider = new webview_provider_1.VibeLearnWebviewProvider(context.extensionUri);
    // Register Sidebar Webview View
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(webview_provider_1.VibeLearnWebviewProvider.viewType, provider));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vibe-learn.internalSaveState', async (state) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vibe-learn.analyze', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vibe-learn.startMission', () => {
        provider.postCommand('startNextMission', {});
    }));
    // Command: Explain File (Context Menu)
    context.subscriptions.push(vscode.commands.registerCommand('vibe-learn.explainFile', (uri) => {
        let filePath = '';
        if (uri) {
            filePath = uri.fsPath;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vibe-learn.explainFunction', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vibe-learn.exportReport', async () => {
        ...
        const config = vscode.workspace.getConfiguration('vibe-learn');
        let state = config.get('playerState');
        if (!state) {
        ...
        features.forEach(f => {
            ...
            f.missions?.forEach((m) => {
                const isDone = m.status === 'completed';
                if (m.pillar === 'architecture') {
                    archTotal++;
                    if (isDone)
                        archDone++;
                }
                else if (m.pillar === 'system_logic') {
                    sysTotal++;
                    if (isDone)
                        sysDone++;
                }
                else if (m.pillar === 'file_logic') {
                    fileTotal++;
                    if (isDone)
                        fileDone++;
                }
                else if (m.pillar === 'function_logic') {
                    fnTotal++;
                    if (isDone)
                        fnDone++;
                }
                else if (m.pillar === 'syntax') {
                    synTotal++;
                    if (isDone)
                        synDone++;
                }
                else if (m.pillar === 'engineering_decisions') {
                    engTotal++;
                    if (isDone)
                        engDone++;
                }
        ...
        const completedConcepts = conceptNodes.filter((n) => n.status === 'completed').length;
        const avgConcepts = Math.round((completedConcepts / totalConcepts) * 100);
        const overallScore = Math.round((avgArch * 0.20) + (avgSys * 0.15) + (avgFile * 0.15) + (avgFn * 0.10) + (avgSyn * 0.10) + (avgEng * 0.15) + (avgConcepts * 0.15));
        const masteredConcepts = conceptNodes.filter((n) => n.status === 'completed').map((n) => `- ${n.name} ✓`);
        const lockedConcepts = conceptNodes.filter((n) => n.status === 'locked').map((n) => {
            const missing = n.prerequisites.filter((p) => {
                const prereqNode = conceptNodes.find((cn) => cn.id === p);
        ...
        for (const p of sortedPillars) {
            ...
            for (const f of features) {
                const nextM = f.missions?.find((m) => m.pillar === p.name.toLowerCase().replace(' ', '_') && m.status !== 'completed');
                if (nextM) {
        ...
        const nextAvailableConcept = conceptNodes.find((n) => n.status === 'available');
...
function updateStatusBar(playerState) {
    if (!statusBarItem) {
        return;
    }
    ...
    if (playerState && playerState.features) {
        const features = Object.values(playerState.features);
